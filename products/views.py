...
def search_results(request):
    ...
    if category_q:
        # Get all matching products
        products = Accessory.objects.filter(categories__name=category_q).distinct() # Use distinct if products might appear multiple times from Q objects
...
def search_pdf(request):
    try:
        ...
        if Accessory.objects.filter(categories__name=query):
...
def shipping_form(request):
    if request.method == 'POST':
        ...
        request.session['final_amount'] = float(total) if mode == "Prepaid" else float(total)*0.9
        total_width = 0
        total_height = 0
        total_weight = 0
        total_length = 0
